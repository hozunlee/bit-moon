...
def load_environment() -> None:
    ...
    if missing_vars:
        error_msg = f"필수 환경변수가 설정되지 않았습니다: {', '.join(missing_vars)}. .env 파일을 확인해주세요."
...
# 데이터베이스 파일 경로
DB_FILE = TestConfig.get_test_db_filename()

def init_db():
    """대시보드와 100% 호환되는 DB 구조로 초기화"""
    try:
        ...
        CREATE TABLE IF NOT EXISTS trades (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            type TEXT NOT NULL,         -- 거래 유형 (buy/sell)
            buy_sell TEXT NOT NULL,     -- 대시보드 호환
            price REAL NOT NULL,        -- 거래 가격
            amount REAL NOT NULL,       -- 거래 수량
            total REAL NOT NULL,        -- 거래 총액
            fee REAL NOT NULL,          -- 수수료
            profit REAL DEFAULT 0,      -- 수익
            ticker TEXT,                -- 거래된 코인
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP  -- 거래 일시
        )''')

        # balance_history 테이블 (대시보드 호환)
        cursor.execute('''
        ...
        conn.commit()
        logger.info("데이터베이스 초기화 완료 (대시보드 호환)")
...
def buy_coin(grid_level):
    ...
    try:
        ...
        # === TEST_MODE: DB에 가상 거래 기록 ===
        if TEST_MODE:
            try:
                conn = sqlite3.connect(DB_FILE)
                cursor = conn.cursor()
                cursor.execute('''
                    INSERT INTO trades (type, buy_sell, price, amount, total, fee, profit, ticker, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    'buy', 'buy', price, volume, order_amount, fee_amount, 0, TICKER, datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                ))
                conn.commit()
                conn.close()
            except Exception as e:
                logger.error(f"TEST_MODE 매수 DB 기록 오류: {str(e)}")
        ...
        if TEST_MODE:
            try:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    'sell', 'sell', price, volume, sell_amount, fee_amount, 0, TICKER, datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                ))
                conn.commit()
                conn.close()
            except Exception as e:
                logger.error(f"TEST_MODE 매도 DB 기록 오류: {str(e)}")
