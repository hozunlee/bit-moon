...
def update_dashboard(TICKER, PRICE_CHANGE, grid_df, metrics_container, grid_container, trades_container):
    ...
    with metrics_container.container():
        ...
        if current_price is not None:
            st.markdown(f"### {TICKER} ({coin_name}) | 현재가: **{current_price:,.0f}원**")
        else:
        ...
        with col2:
            ...
            if not grid_df.empty and 'order_krw_amount' in grid_df.columns:
                ...
                if pd.notna(order_krw_amount_value) and order_krw_amount_value > 0 and num_grid_levels > 0:
                    ...
                    profit_percentage_on_grid_text = "계산 불가"
            else:
                profit_percentage_on_grid_text = "정보 없음"

            current_delta_text = f"수수료: {total_fees:,.0f}원{format_korean_won(total_fees, with_space=False)}"
            if profit_percentage_on_grid_text:
                 current_delta_text += f" | {profit_percentage_on_grid_text}"
            
            st.metric(
                "총 수익",
                f"{total_profit:,.0f}원{format_korean_won(total_profit)}",
        ...
        with col3:
            if not latest_balance.empty:
                ...
                if not balance_df.empty and len(balance_df) > 1:
                    ...
                    delta_text = f"{assets_change:+,.0f}원{format_korean_won(assets_change)} ({assets_change_pct:+.2f}%)"
                else:
                    delta_text = "변화 없음"

                st.metric(
                    "현재 총 자산",
                    f"{current_assets:,.0f}원{format_korean_won(current_assets)}",
                    delta_text
                )
    ...
    with grid_container.container():
        ...
        st.markdown(
            ...
            @keyframes colorTransition {{
                0% {{ background: linear-gradient(45deg, #606060, #505050); }}
                100% {{ background: linear-gradient(45deg, #9c9c9c, #8c8c8c); }}
            }}
            </style>
            """,
        ...
        if not grid_df.empty:
            ...
            grid_df_display = grid_df_display.rename(columns={
                'grid_level': '구간', 'buy_price_target': '매수목표가', 'sell_price_target': '매도목표가',
                'order_krw_amount': '주문금액', 'is_bought': '매수상태', 'actual_bought_volume': '매수수량',
                'actual_buy_fill_price': '매수가격', 'timestamp': '최종업데이트'
            })
            
            grid_df_display['매수목표가'] = grid_df_display['매수목표가'].apply(lambda x: f"{x:,.0f}원")
            grid_df_display['매도목표가'] = grid_df_display['매도목표가'].apply(lambda x: f"{x:,.0f}원")
            ...
            grid_df_display['매수가격'] = grid_df_display['매수가격'].apply(lambda x: f"{x:,.0f}원" if x > 0 else "-")
            ...
            def add_arrow_to_current_grid(row):
                try:
                    ...
                    # 매도 목표가는 다음 그리드의 매수 목표가와 같다고 가정
                    sell_target = buy_target + PRICE_CHANGE 
            ...
            st.dataframe(
                grid_df_display[display_columns],
    ...
    with trades_container.container():
        ...
        trades_df = load_trades(7, TICKER)
        
        if not trades_df.empty:
            trades_df = trades_df.rename(columns={
                'timestamp': '시간', 'buy_sell': '거래유형', 'grid_level': '그리드레벨',
                'price': '가격', 'amount': '거래금액', 'volume': '거래수량',
                'fee': '수수료', 'profit': '수익', 'profit_percentage': '수익률'
            })
            
            trades_df['시간'] = pd.to_datetime(trades_df['시간']).dt.strftime('%Y-%m-%d %H:%M:%S')
            trades_df['가격'] = trades_df['가격'].apply(lambda x: f"{x:,.0f}원")
            trades_df['거래금액'] = trades_df['거래금액'].apply(lambda x: f"{x:,.0f}원{format_korean_won(x)}")
            trades_df['수수료'] = trades_df['수수료'].apply(lambda x: f"{x:,.0f}원{format_korean_won(x)}")
            trades_df['수익'] = trades_df['수익'].apply(lambda x: f"{x:,.0f}원{format_korean_won(x)}")
